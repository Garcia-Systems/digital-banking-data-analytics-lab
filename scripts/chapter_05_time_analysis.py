...
if __name__=='__main__':
 ...
 print('day        starts completed completion identity-success mobile-events')
 for day, row in completion.items():
  mobile=sum(x['channel']=='mobile' for x in e if x['timestamp'].startswith(day))
  print(f"{day} {starts[day]:6} {completions[day]:9} {row['rate']:9.1f}% {identity[day]['rate']:14.1f}% {mobile:13}")
