...
print(f"[KPI candidate] observed application completion rate: {rate(len(completions), len(starts)):.1f}%")
for channel in ("mobile", "web"):
    ...
    print(f"[dimension breakdown] {channel} completion rate: {rate(len(channel_completions), len(channel_starts)):.1f}%")
...
print(f"[KPI candidate] identity-verification success rate: {rate(identity_success, identity_starts):.1f}%")
