...
# Chapter 23 capstone sources

All `capstone_*` CSVs are synthetic. `capstone_journey_events` contains event time, session/application IDs, stage, channel/device/campaign, optional request/correlation IDs, and fictional app version. `capstone_api_requests` contains endpoint, method, status and duration at request grain. `capstone_vendor_calls` records one row per fictional-provider attempt (including attempt and outcome), so it must not be counted as member-visible operations without grouping by `correlation_id`.

`capstone_database_observations` records query category, duration, rows examined, and a slow flag; `capstone_errors` records layer/code/recovery; `capstone_navigation` records entry/campaign/search observations; and `capstone_releases` records non-sensitive fictional version metadata. These tables join through synthetic `session_id`, `application_id`, `request_id`, and `correlation_id`; none contains member profile, payload, credential, or financial data.
